...
:G4VSensitiveDetector(name)
{
	...
	ss_filename<<"Detector_2_"<<year << month << hour << day << "_" << hour << ":" << min <<":" <<sec <<"_RankNum:" << rank;
...
G4bool SensitiveDetector_add::ProcessHits(G4Step* aStep, G4TouchableHistory*)
{
	...
	if(aStep->GetTrack()->GetParentID() != 0 ){
		if(aStep->GetTrack()->GetParticleDefinition()->GetParticleName()== "gamma"){
			...
			if (gamma_E/keV > 510 && gamma_E/keV < 512){
			//if (aStep->GetTrack()->GetCreatorProcess()->GetProcessName()=='annihil'){
			...
			ofs<<X/cm<<tab<<Y/cm<<tab<<Z/cm<<tab<<X_mom<<tab<<Y_mom<<tab<<Z_mom<<tab<<gamma_E/MeV<<tab<<PDG_type<<tab<<TOF<<tab<<Origin_Volume<<G4endl;
