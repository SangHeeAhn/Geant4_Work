...
CML2Ph_FullWater::CML2Ph_FullWater()
{
	...
	this->halfSize.set(200.*mm,200.*mm,200.*mm);
	// phantom position
...
// 	simpleAlSVisAtt->SetForceSolid(true);
	...
	// Sensitive detector 
	this->sensDet=new CML2SDWithVoxels("Water phantom", saving_in_ROG_Voxels_every_events, seed, ROGOutFile, bSaveROG, G4ThreeVector(0.,0.,0.), halfSize, 200, 200, 200);
	G4SDManager *SDManager=G4SDManager::GetSDMpointer();
	SDManager->AddNewDetector(this->sensDet);
	
	// Read Out Geometry
	CML2ReadOutGeometry *ROG = new CML2ReadOutGeometry();
	ROG->setBuildData(this->PVWorld->GetFrameTranslation(), halfSize, 200, 200, 200);
	ROG->BuildROGeometry();
